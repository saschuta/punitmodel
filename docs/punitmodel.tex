...
\begin{table}[h!]
  \begin{center}
    ...
    \begin{tabular}{l|c|l}
      ...
      $\tau_{m}$  & membrane time constant &  \,[s]\\       
      $\mu$  & bias current &  \,[mV]\\      
      $\sqrt{2D}$  & noise strength &  \, [mV$\sqrt{\textup{s}}$] \\      
      $\tau_{A}$  & adaption time constant &  \,[s]\\      
      $\Delta A$  & adaption strength &  \,[mVs]\\      
      $\tau_{d}$  & time constant of dendritic low-pass filter &  \,[s]\\      
      $t_{ref}$  & absolute refractory period &  \,[s]\\     
	  $\Delta t$  & time step &  \,[s]\\           
    \end{tabular}
  \end{center}
\end{table}
